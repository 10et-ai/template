# tenet-template - Architecture

## Overview

tenet-template is a git repository that serves as the canonical starter template for all TENET GTM workspaces. It is not a running service -- it is a static template that gets cloned, customized, and then operates independently as a new project. The architecture is entirely file-based: markdown documents, shell scripts, JSON configuration, and Claude Code skill definitions.
...
├── .mcp.json                          # MCP server config for tenet-context-hub
...
│   │   ├── tenet-doctor.sh              # Health checks with --fix mode
...
- **Purpose:** The single source of truth for how Claude Code behaves in a TENET project
...
This file is the most critical file in the template. When `tenet update` runs, it replaces this file with the latest version from the template.
...
Session Start (session-init.sh)
    ...
    ├── 5. Concurrent session detection (via tenet-services API or local)
    ...
    └── 7. Save session ID to .tenet/current-session-branch.txt
...
Configuration stored in `.tenet/config.json` under the `crm` key. Includes an interactive setup wizard (`./crm setup`).
...
When `tenet init` creates a new project from this template, these are the primary customization points:

| What Gets Customized | How |
|---------------------|-----|
| `.tenet/config.json` | `name`, `type`, `team.owner`, `description`, `product_repo` filled in |
...
## How It Connects to `tenet init`

The `tenet init` command (implemented in the tenet-platform CLI) performs:

1. `git clone git@github.com:402goose/tenet-template.git <project-name>`
2. Removes the `.git` directory and re-initializes
3. Updates `.tenet/config.json` with project name and owner info
4. Optionally adds a product submodule
5. Makes initial commit

## How `tenet update` Works

The `tenet update` command refreshes template files in an existing project:

1. Fetches latest from 402goose/tenet-template
...
6. Does NOT touch `knowledge/` (user content), `content/`, `suggestions/`, or `.tenet/config.json`

## Known Constraints

- The template's `.tenet/config.json` has its own `gtm_parent` set to `/Users/alectaggart/CascadeProjects/JFL-GTM`, which is a local path specific to the development machine. This gets overwritten during `tenet init`.
...
- The tenet-services API on localhost:3401 is optional -- scripts gracefully fall back to local detection.
